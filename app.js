...
const API = (function() {
  // Works for XAMPP (any subfolder) and Railway (root)
  // URL example: http://localhost/SoftLife_Updated/signup.php
  // We need:     http://localhost/SoftLife_Updated/api
  const parts = window.location.pathname.split('/').filter(p => p !== '');
  // Remove the filename (last part if it contains a dot)
  if (parts.length > 0 && parts[parts.length - 1].includes('.')) {
    parts.pop();
  }
  const base = parts.length > 0 ? '/' + parts.join('/') : '';
  const apiPath = window.location.origin + base + '/api';
  return apiPath;
})();
...
function showPage(pageId) {
  // In multi-page mode, navigate to separate PHP files instead
  if (window.__SOFTLIFE_MULTIPAGE__) {
    var map = {homePage:'index.php', aboutPage:'about.php', loginPage:'login.php', signupPage:'signup.php', dashPage:'dashboard.php', contactPage:'contact.php'};
    if (map[pageId]) { location.href = map[pageId]; return; }
  }
...
function refreshDashboard() {
  ...
  const name = SL.currentUser?.username || localStorage.getItem('sl_username') || 'Friend';
  if (SL.currentUser?.username) localStorage.setItem('sl_username', SL.currentUser.username);
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Multi-page mode: do NOT auto-navigate. Each PHP page handles its own active state.
  // Just restore theme and make sure current page stays visible.
  const token = localStorage.getItem('sl_token');
  const isMultiPage = window.__SOFTLIFE_MULTIPAGE__ === true;
  if (isMultiPage) {
    // Keep the page that PHP already marked as active
    applyTheme(localStorage.getItem('sl_gender') || 'default');
    // If on dashboard page, load data
    if (document.getElementById('dashPage') && document.getElementById('dashPage').classList.contains('active')) {
      if (token) {
        await loadState();
        if (SL.currentUser) {
          applyTheme(SL.gender);
...
function toggleMobileNav() {
  var nav = document.getElementById('homeNavLinks');
  var btn = document.getElementById('homeHamburger');
  if (!nav || !btn) return;
